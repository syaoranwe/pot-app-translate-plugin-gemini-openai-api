...
pub fn translate(
    ...
    let request_url = needs.get("request_url").ok_or("缺少必要参数: request_url: 自定义请求地址")?;
    let model_string = needs.get("model_string").ok_or("缺少必要参数: model_string: 自定义模型名")?;
    ...
    let payload = json!({
        ...
        "max_output_tokens": 2048  // Gemini-1.5-Flash模型吐字最低约90Token/s，2K Token约22s，30s内若无返回则认为超时
    });

    // 发送请求并处理响应; 请求的url == request_url
    let client = reqwest::blocking::ClientBuilder::new()
        .timeout(std::time::Duration::from_secs(30))  // 设置请求超时时间为30秒
        .build()?;
    let response = client
        .post(request_url)
...
mod tests {
    ...
    fn try_request() {
        ...
        needs.insert("request_url".to_string(), "https://openrouter.ai/api/v1/chat/completions".to_string());
        needs.insert("model_string".to_string(), "google/gemini-flash-1.5".to_string());
